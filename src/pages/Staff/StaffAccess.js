...
const StaffAccess = () => {
  ...
  const { id, userId, user, tempUser } = location.state;
  ...
  const { data: accessData, status } = useQuery({
    queryKey: ["accessModules", id],
    queryFn: async () => {
      const { data } = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/api/v1/user/access-control/staff/${id}`,
        jsonHeader()
      );
      return data?.data;
    },
  ...
  const { mutate } = useMutation({
    ...
    onSuccess: (data) => {
      if (data?.data?.success) {
        ...
        queryClient.invalidateQueries(["accessModules", id]);
      }
    },
    onSettled: () => {
      setIsLoading(false);
