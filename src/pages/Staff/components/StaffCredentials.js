...
import { StaffLoader } from "../StaffVerify";

const StaffCredentials = ({ staff }) => {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      const response = await axios.patch(
        `${process.env.REACT_APP_BASE_URL}/api/v1/user/staffs/reset/password/${staff?.id}`,
        body,
        jsonHeader()
      );
...
const ChangeEmailAndPhone = ({ isOpen, staff, setIsOpen }) => {
  ...
  const handleSendOtp = async () => {
    try {
      ...
      const endPoint = isEmail
        ? `/api/v1/user/staffs/reset/email/${staff?.id}`
        : `/api/v1/user/staffs/reset/phone/${staff?.id}`;
  ...
  const handleResendOtp = async () => {
    try {
      ...
      const endPoint = isEmail
        ? `/api/v1/user/staffs/reset/otp/email/resend/${staff?.id}`
        : `/api/v1/user/staffs/reset/otp/phone/resend/${staff?.id}`;
  ...
  const handleOtpVerify = async () => {
    try {
      ...
      const endPoint = isEmail
        ? `/api/v1/user/staffs/reset/otp/email/verify/${staff?.id}`
        : `/api/v1/user/staffs/reset/otp/phone/verify/${staff?.id}`;
