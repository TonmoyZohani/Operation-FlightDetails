  ...
  const {
    ...
    queryFn: async () => {
      const { data } = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/v1/common/statics`, {
        params: {
          wing: "FFI",
          field: activeTab === 0 ? "refundPolicy" : "refundPolicy-bn",
        },
      });
      return data?.data[0];
    },
        ...
        >
          ...
          >
            <Typography sx={style.header}>
              <span style={{ fontWeight: 100 }}>REFUND & </span> CANCELLATION
              POLICY
            </Typography>
            <Box sx={{ width: "170px" }}>
              <CustomTabBar
...
export default RefundCancelationPolicy;
